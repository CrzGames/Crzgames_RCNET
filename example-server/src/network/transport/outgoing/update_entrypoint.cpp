...
#include <RCNET/RCNET.h> // for RCNET_log and log levels
...
{
    // Vérifier que le message est bien du type snapshot full unreliable avant de continuer.
    if (msg.type != SimulationToNetworkOUTMessageType::SERVER_SNAPSHOT_FULL_PACKET_UNRELIABLE)
        return;

    // Trouver la session du client correspondant à cette connectionId pour mettre à jour le dernier snapshot envoyé.
    std::unordered_map<uint32_t, ClientSession>::iterator sit = networkState.sessions.find(msg.connectionId);

    // Si aucune session n’existe pour cette connectionId, on ne peut pas envoyer ce snapshot.
    if (sit == networkState.sessions.end())
        return;

    ClientSession& session = sit->second;

    // Désérialiser la version construite côté simulation.
            ...
            snapshotFullPacket))
    {
        RCNET_log(RCNET_LOG_WARN,
                  "[SERVER] [NETWORK_OUT] [SNAPSHOT_FULL_UNRELIABLE] - Failed to deserialize snapshot for connectionId=%u\n",
                  msg.connectionId);
        return;
    }
    ...
    // Mémoriser le dernier snapshot réellement envoyé dans la session pour ce client.
