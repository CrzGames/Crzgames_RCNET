...
static bool ClientNetworkEncryption_TryGetConnectionIdFromPeer(const ENetPeer* peer, uint32_t& outConnectionId)
{
...
static bool ClientNetworkEncryption_TryLoadSessionKeyIfEnabled(
    ...
    size_t outLimit)
{
    ...
    for (size_t i = 0; i < inBufferCount && copied < inLimit; ++i)
    {
        ...
        if (inBuffers[i].data == nullptr)
        {
            RCNET_log(
                RCNET_LOG_ERROR,
                "[CLIENT] [NETWORK_ENCRYPTION] [COPY_BUFFERS] - inBuffers[%u].data is null",
    ...
    size_t outLimit)
{
    ...
    if (!ClientNetworkEncryption_TryGetConnectionIdFromPeer(peer, connectionId))
    {
        return 0;
    }
    ...
    if (!ClientNetworkEncryption_TryLoadSessionKeyIfEnabled(networkState, connectionId, true, txKey))
    {
        return 0;
    }
    ...
    if (requiredSize > outLimit)
    {
        RCNET_log(
            RCNET_LOG_ERROR,
            "[CLIENT] [NETWORK_ENCRYPTION] [ENCRYPT] - output buffer too small for connectionId=%u (required=%zu outLimit=%zu)",
            connectionId,
            requiredSize,
            outLimit);
            ...
            plaintext.size()) != inLimit)
    {
        RCNET_log(
            RCNET_LOG_ERROR,
            "[CLIENT] [NETWORK_ENCRYPTION] [ENCRYPT] - failed to linearize input buffers for connectionId=%u",
            connectionId);
        sodium_memzero(txKey.data(), txKey.size());
    ...
    if (encryptResult != 0)
    {
        RCNET_log(
            RCNET_LOG_ERROR,
            "[CLIENT] [NETWORK_ENCRYPTION] [ENCRYPT] - crypto_aead_xchacha20poly1305_ietf_encrypt failed for connectionId=%u",
            connectionId);
        return 0;
    }
    ...
    size_t outLimit)
{
    ...
    if (!ClientNetworkEncryption_TryLoadSessionKeyIfEnabled(networkState, connectionId, false, rxKey))
    {
        return 0;
    }
    ...
    if (inData == nullptr || inLimit < (kEncryptedPacketHeaderSize + kEncryptedPacketTagSize))
    {
        RCNET_log(
            RCNET_LOG_WARN,
            "[CLIENT] [NETWORK_ENCRYPTION] [DECRYPT] - invalid packet size/pointer for connectionId=%u (inData=%p inLimit=%zu min=%zu)",
            connectionId,
            inData,
            inLimit,
    ...
    if (std::memcmp(inData, kEncryptedPacketMagic, kEncryptedPacketMagicSize) != 0)
    {
        ...
        RCNET_log(
            RCNET_LOG_WARN,
            "[CLIENT] [NETWORK_ENCRYPTION] [DECRYPT] - Invalid encrypted packet magic for connectionId=%u",
            connectionId);
        return 0;
    }
    ...
    if (maximumPlaintextLen > outLimit)
    {
        RCNET_log(
            RCNET_LOG_WARN,
            "[CLIENT] [NETWORK_ENCRYPTION] [DECRYPT] - output buffer too small for connectionId=%u (maxPlaintext=%zu outLimit=%zu)",
            connectionId,
            maximumPlaintextLen,
            outLimit);
        sodium_memzero(rxKey.data(), rxKey.size());
    ...
    if (decryptResult != 0)
    {
        RCNET_log(
            RCNET_LOG_WARN,
            "[CLIENT] [NETWORK_ENCRYPTION] [DECRYPT] - Authentication failed for connectionId=%u",
            connectionId);
        return 0;
    }
...
void ClientNetworkEncryption_EnsureHostEncryptorInstalled(ENetHost* host)
{
    ...
    if (host == nullptr)
    {
        RCNET_log(
            RCNET_LOG_ERROR,
            "[CLIENT] [NETWORK_ENCRYPTION] - host is null, encryptor install skipped");
        return;
    }
    ...
    RCNET_log(RCNET_LOG_INFO, "[CLIENT] [NETWORK_ENCRYPTION] - ENet host encryptor installed");
}
